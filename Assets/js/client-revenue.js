        ...
        mQuery('#client-revenue:first:not(.table-initialized)').addClass('table-initialized').each(function() {
            ...
            mQuery.ajax({
                ...
                success: function(response) {
                    ...
                    var rowCount = Math.floor(($clienttarget.data('height') - (235 + cpaginate)) / 40);
                    var colAdjust = $clienttarget.data('groupby') == 'Client Category' ? 4 : 6;
                    var order = $clienttarget.data('groupby') == 'Client Category' ? [
                        [2, 'asc'],
                        [3, 'asc']
                    ] : [
                        [2, 'asc'],
...
mQuery.getScriptCachedOnce = function(url, callback) {
    if (
        typeof window.getScriptCachedOnce !== 'undefined' && window.getScriptCachedOnce.indexOf(url) !== -1
    ) {
        callback();
        return mQuery(this);
    } else {
