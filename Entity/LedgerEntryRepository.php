...
class LedgerEntryRepository extends CommonRepository
{
    ...
    {
        $fromDate = isset($params['dateFrom']) ? $params['dateFrom']->format('Y-m-d H:i:s') : null;
        $toDate   = isset($params['dateTo']) ? $params['dateTo']->format('Y-m-d H:i:s') : null;
        $results  = [];
        ...
        if ($fromDate) {
            $q->where(
                $q->expr()->gte('l.date_added', ':fromDate')
            );
        ...
        if ($toDate) {
            if (!$fromDate) {
                $q->where(
                    $q->expr()->lte('l.date_added', ':toDate')
                );
            } else {
                $q->andWhere(
                    $q->expr()->lte('l.date_added', ':toDate')
        ...
        $results['summary']['count']          = count($financials);
